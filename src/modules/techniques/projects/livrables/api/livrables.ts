...
export const useLivrableService = () => {
   ...
   */
  const getAllLivrables = async (): Promise<Livrable[]> => {
    try {
      const response = await apiClient.get(`/technique/livrables`);
   ...
   */
  const getLivrableById = async (id: number): Promise<Livrable | undefined> => {
    try {
      console.log(
   ...
   */
  const deleteLivrable = async (id: number): Promise<void> => {
    try {
      console.log(
      ...
      if (associatedDocuments.length > 0) {
        ...
        console.log(
          `[API Livrables] Livrable ${id} n'a aucun document associé.`
        );
      }
   ...
   */
  const addDocumentToLivrable = async (
    livrableId: number,
    documentFile: File,
   ...
   */
  const getDocumentsByLivrableId = async (
    livrableId: number
   ...
   * @returns Promesse résolue une fois la suppression effectuée.
   */
  const deleteDocumentFromLivrable = async (
    livrableId: number,
    documentId: number
  ): Promise<void> => {
    try {
   ...
   * @returns Promesse résolue avec l'objet ApiResponse contenant un tableau de Livrable et la pagination.
   */
  const getLivrablesByProjetId = async (
    projetId: number
   ...
   */
  const getAllNatureDocuments = async (): Promise<ApiResponse<Nature[]>> => {
    try {
      console.log(
