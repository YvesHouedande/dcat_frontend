...
import { useProjetService } from "../projet/api/projets"; // Chemin correct pour l'API des projets
...
const LivrablesPage: React.FC = () => {
  ...
  const { getAllLivrables, deleteLivrable } = useLivrableService();
  ...
  const [filterApprobation, setFilterApprobation] = useState<
    Livrable["approbation"] | "tous"
  >("tous");
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        if (Array.isArray(livrablesResponse)) {
          ...
          console.log(
            "[LivrablesPage] Livrables récupérés:",
            livrablesResponse
          );
        } else {
          console.warn(
            "[LivrablesPage] Structure de réponse livrables inattendue:",
            livrablesResponse
          );
          setLivrables([]); // S'assure que c'est un tableau vide en cas de structure inattendue
          toast.warning(
            "Impossible de charger les livrables. Structure de réponse inattendue."
          );
        }
        ...
        if (
          projetsResponseRaw &&
          typeof projetsResponseRaw === "object" &&
          ("data" in projetsResponseRaw || "success" in projetsResponseRaw)
        ) {
          const apiResponse = projetsResponseRaw as ApiResponse<Projet[]>;
          if (apiResponse.data && Array.isArray(apiResponse.data)) {
            projectsToSet = apiResponse.data;
            console.log(
              "[LivrablesPage] Projets récupérés via ApiResponse.data:",
              projectsToSet
            );
          } else {
            console.warn(
              "Structure d'ApiResponse inattendue pour la récupération des projets, données manquantes:",
              projetsResponseRaw
            );
            toast.warning(
              "Impossible de charger les projets. Données de l'API inattendues."
            );
          }
        } else if (Array.isArray(projetsResponseRaw)) {
          projectsToSet = projetsResponseRaw;
          console.log(
            "[LivrablesPage] Projets récupérés directement sous forme de tableau:",
            projectsToSet
          );
        } else {
          console.warn(
            "Structure de réponse inattendue pour la récupération des projets: ni ApiResponse ni un tableau direct.",
            projetsResponseRaw
          );
          toast.warning(
            "Impossible de charger les projets. Structure de réponse inattendue."
          );
        }
        setProjets(projectsToSet);
      } catch (err) {
        console.error(
          "Erreur lors du chargement des données des livrables :",
          err
        );
    ...
    loadData();
  }, [fetchAllProjets, getAllLivrables]); // [] pour un chargement au montage uniquement
  ...
  const filteredLivrables = useMemo(() => {
    return livrables.filter((livrable) => {
      const matchesSearch =
        ...
        (livrable.realisations &&
          livrable.realisations
            .toLowerCase()
            .includes(searchTerm.toLowerCase())) ||
        (livrable.reserves &&
          livrable.reserves.toLowerCase().includes(searchTerm.toLowerCase()));

      const matchesApprobation =
        filterApprobation === "tous" ||
        livrable.approbation === filterApprobation;
      const matchesProjet =
        filterProjet === 0 || livrable.id_projet === filterProjet;
  ...
  const handleDelete = async (id: number) => {
    ...
    toast.promise(
      async () => {
        ...
        setLivrables((prev) =>
          prev.filter((livrable) => livrable.id_livrable !== id)
        );
  ...
  const kpis = useMemo(() => {
    ...
    livrables.forEach((livrable) => {
      ...
      if (
        ...
        livrable.approbation in livrablesByApprobation
      ) {
      ...
      if (
        livrable.date &&
        livrable.approbation !== "approuvé" &&
        livrable.approbation !== "rejeté"
      ) {
  ...
  return (
    <div className="p-6 min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <LivrableHeader
          onAddLivrable={() =>
            navigate("/gestion-des-projets/projets/livrables/nouveau")
          }
        />
        ...
        {loading ? (
          ...
          <>
            <LivrableTable
              ...
              onEdit={(id) =>
                navigate(`/gestion-des-projets/projets/livrables/${id}/editer`)
              }
              onView={(id) =>
                navigate(`/gestion-des-projets/projets/livrables/${id}/details`)
              }
