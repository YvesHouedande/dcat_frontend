...
import { BarChart3, Clock, CheckCircle, RefreshCcw, Coins, FileText, Plus } from "lucide-react"; 
...
const ProjetsPage = () => {
  ...
  const totalPages = serverPagination ? serverPagination.totalPages : Math.ceil(projets.length / projetsPerPage);
  ...
  console.log('Projets sur cette page:', currentProjets.map(p => `${p.id_projet}: ${p.nom_projet}`));
  ...
  const handleDelete = useCallback(async (id: number) => {
    ...
    try {
      ...
      if (result.success) {
        toast.success(`Projet "${projetToDelete.nom_projet}" supprimé avec succès !`); // Utilisation de toast
        ...
        if (currentPage > Math.ceil((projets.length - 1) / projetsPerPage) && currentPage > 1) {
      ...
      toast.error(`Erreur lors de la suppression: ${err.message || "Erreur inconnue."}`); // Utilisation de toast
    } finally {
      setLoading(false); 
    }
  }, [projets, loadAllData, currentPage, projetsPerPage]); // Dépendances pour `useCallback`
  ...
  const kpis = {
    ...
    budgetMoyen: projets.length > 0 ? projets.reduce((sum, p) => sum + Number(p.devis_estimatif), 0) / projets.length : 0
  };
      ...
      <div className="container mx-auto py-6 space-y-6">
        ...
        {error && (
          <Alert variant="destructive">
            ...
            <AlertDescription>
              {error}
              <Button variant="ghost" onClick={handleRefresh} className="ml-2 px-2 py-1 h-auto text-sm">
