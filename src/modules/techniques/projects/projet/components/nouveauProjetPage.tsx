...
const NouveauProjetPage = () => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (Array.isArray(fetchedNatureDocumentsRaw)) {
          ...
          console.log(
            "[NouveauProjetPage] Natures de document récupérées directement sous forme de tableau:",
            fetchedNatureDocumentsRaw
          );
        } else {
          console.warn(
        ...
        toast.error(
          "Impossible de charger les listes (partenaires, familles, employés, natures de document)."
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleSaveProjet = async (
    ...
    try {
      ...
      const apiResponse = await createProjet(
        projetToCreate as Omit<Projet, "id_projet" | "id_partenaire">
      );
      ...
      if (!newProjet.id_projet || newProjet.id_projet === 0) {
        toast.error(
          "Erreur: L'ID du projet créé est manquant. Impossible d'associer les partenaires/documents."
        );
      ...
      toast.success(
        `Projet "${newProjet.nom_projet}" créé avec succès ! Vous pouvez maintenant y associer des documents.`
      );
      ...
      if (id_partenaire && id_partenaire.length > 0) {
        ...
        const results = await Promise.allSettled(
          id_partenaire.map((partnerId) =>
            addPartenaireToProjet(newProjet.id_projet, partnerId)
          )
        );
        ...
        results.forEach((result, index) => {
          if (result.status === "fulfilled") {
            ...
            console.error(
              `Échec de l'ajout du partenaire ${partnerId} au projet ${newProjet.id_projet}:`,
              result.reason
            );
          }
        });
        ...
        if (partnersAddedCount > 0) {
          toast.success(
            `${partnersAddedCount} partenaire(s) associé(s) au projet.`
          );
        }
