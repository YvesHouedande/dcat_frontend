...
import { useProjetService } from "../api/projets";
import { usePartenairesApi } from "../api/partenaires";
import { getFamilles } from "../api/famille";
import { useEmployesApi } from "../api/employes";
import { useLivrableService } from "../../livrables/api/livrables"; // Import the function to get document natures
...
const EditerProjetPage = () => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (Array.isArray(fetchedNatureDocumentsRaw)) {
          ...
          console.log(
            "[EditerProjetPage] Natures de document récupérées directement sous forme de tableau:",
            fetchedNatureDocumentsRaw
          );
        } else {
          console.warn(
            "Structure de réponse inattendue pour la récupération des natures de document: n'est pas un tableau direct.",
            fetchedNatureDocumentsRaw
          );
          toast.warning(
            "Impossible de charger les natures de document. Structure de réponse inattendue."
          );
        }
      } catch (err) {
        console.error(
          "Erreur lors du chargement des données pour l'édition:",
          err
        );
  ...
  const handleSaveProjet = async (projetMisAJour: Projet) => {
    try {
      if (!projetMisAJour.id_projet || projetMisAJour.id_projet === 0) {
        console.error(
          "Tentative de sauvegarde d'un nouveau projet sur la page d'édition."
        );
        toast.error(
          "Erreur: Tentative de créer un nouveau projet sur la page d'édition."
        ); // Use toast
        return;
      }
      ...
      console.log(
        "Informations de base du projet mises à jour :",
        projetMisAJour
      );
      ...
      const partnersToAdd = newPartnerIds.filter(
        (id) => !initialPartnerIds.includes(id)
      );
      for (const partnerId of partnersToAdd) {
        await addPartenaireToProjet(projetMisAJour.id_projet, partnerId);
        console.log(
          `Partenaire ${partnerId} ajouté au projet ${projetMisAJour.id_projet}`
        );
      }

      const partnersToRemove = initialPartnerIds.filter(
      ...
      for (const partnerId of partnersToRemove) {
        ...
        console.log(
          `Partenaire ${partnerId} désassocié du projet ${projetMisAJour.id_projet}`
        );
      }
      ...
      console.error(
        "Erreur lors de la sauvegarde du projet ou de la gestion des partenaires:",
        err
      );
      toast.error(
        `Échec de la mise à jour du projet: ${(err as Error).message}`
      ); // Use toast
    }
  };
  ...
  const handleSaveDocument = async (
    ...
    try {
      ...
      console.error(
        "Erreur lors de l'association du document au projet :",
        err
      );
