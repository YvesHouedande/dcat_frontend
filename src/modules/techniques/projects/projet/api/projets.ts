...
import { useLivrableService } from "../../livrables/api/livrables"; // Chemin relatif vers src/api/livrables.ts
...
export const useProjetService = () => {
  ...
  const { deleteLivrable, getLivrablesByProjetId } = useLivrableService();
  ...
  const addPartenaireToProjet = async (
    projectId: number,
    partnerId: number
  ): Promise<Partenaire> => {
    try {
  ...
  const getProjetAssociatedPartenaires = async (
    projectId: number
  ): Promise<number[]> => {
    try {
      console.log(
  ...
  const updateProjet = async (projetData: Projet): Promise<Projet> => {
    try {
      console.log(
      ...
      if (!projetMisAJour || !projetMisAJour.id_projet) {
        ...
        throw new Error(
          "Structure de réponse API inattendue après mise à jour"
        );
      }
  ...
  const removePartenaireFromProjet = async (
    projectId: number,
    partnerId: number
  ): Promise<Partenaire> => {
    try {
  ...
  const deleteProjet = async (
    ...
    try {
      ...
      if (associatedDocuments.length > 0) {
        ...
        const documentDeletionPromises = associatedDocuments.map(
          async (doc) => {
            try {
              console.log(
        ...
        const dissociationPromises = associatedPartenaires.map(
          async (partnerId) => {
            try {
              ...
              console.log(
                `[API] ✓ Partenaire ${partnerId} dissocié avec succès`
              );
      ...
      if (!apiResponse.data || !apiResponse.data.id) {
        console.warn(
          `[API] ID du projet supprimé non retourné dans la réponse`
        );
      }
      const result = {
        success: true,
  ...
  const deleteProjetWithConfirmation = async (
    projectId: number,
    projectName: string
  ): Promise<{ success: boolean; message: string; deletedId: number }> => {
    try {
  ...
  const fetchAllProjets = async (
    ...
    try {
      ...
      if (partenaireId && partenaireId !== 0)
        params.partenaire_id = partenaireId;
   ...
   */
  const getDocumentsByProjetId = async (
    projectId: number
   ...
   */
  const addDocumentToProjet = async (
    projectId: number,
    documentFile: File,
   ...
   */
  const deleteDocumentFromProjet = async (
    projectId: number,
    documentId: number
  ): Promise<void> => {
    try {
   ...
   */
  const getLivrablesWithDocumentsByProjetId = async (
    projectId: number
