...
export const ExemplairesManager = () => {
  ...
  const [currentExemplaire, setCurrentExemplaire] = useState<Exemplaire | null>(null);
  ...
  const handleSubmit = (data: ExemplaireFormValues) => {
    if (currentExemplaire) {
      ...
      addExemplaire({ ...data, id_exemplaire: generateExemplaireId() } as Exemplaire);
    }
  ...
  const stats = {
    ...
    available: exemplaires.filter(ex => ex.etat_disponible_indisponible_ === 'disponible').length,
    unavailable: exemplaires.filter(ex => ex.etat_disponible_indisponible_ === 'indisponible').length
  };
