...
import { useExemplairesOutils } from "../exemplaire/hooks/ExemaplaireOutils";
import { useDebounce } from "@/modules/stocks/entree/utils/helpers";
interface OutilsInstanceTableProps {
  onEdit: (instance: OutilEntreeType) => void;
  onDelete: (id: string | number) => void;
  onAdd: () => void;
}

export function OutilsInstanceTable({
  onEdit,
  onDelete,
  onAdd,
}: OutilsInstanceTableProps) {
  ...
  const pageInstances = ExemplaireOutils.slice(
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
        ...
        <Button onClick={onAdd} variant={"blue"} className="w-full sm:w-auto">
          <Plus className="mr-2 h-4 w-" /> Ajouter un Outil
        </Button>
      </div>
        ...
        <Table>
          <TableHeader className="bg-gray-50">
            <TableRow>
              ...
              <TableHead className="font-semibold">Outil</TableHead>
              ...
              <TableHead className="font-semibold">Date de retour</TableHead>
            ...
            {isLoading || isFetchingNextPage ? (
              ...
              pageInstances.map((instance) => (
                <TableRow key={instance.id_exemplaire}>
                  <TableCell>
                    <div className="w-12 h-12 bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl flex items-center justify-center shadow-lg">
                      {instance.image_produit ? (
                        <ImageOrIcon src={instance.image_produit.url} />
                      ) : (
                  ...
                  <TableCell>{instance.date_sortie_outil}</TableCell>
                  <TableCell>{instance.date_retour_outil}</TableCell>
                  <TableCell>{instance.etat_exemplaire}</TableCell>
                  <TableCell>{instance.fournisseur.nom}</TableCell>
                  <TableCell>
                    <DropdownMenu>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          <Button
            ...
            disabled={currentPage <= 1 || isLoading || isFetchingNextPage}
          >
            Précédent
          </Button>
