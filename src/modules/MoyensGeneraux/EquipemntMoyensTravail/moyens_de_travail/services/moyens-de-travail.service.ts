...
// Hook personnalisé pour les moyens de travail
export const useMoyensDeTravailService = () => {
  const api = useApi();

  return {
    getAll: async (
      _filters: MoyensFilters
    ): Promise<PaginatedResponse<MoyenDeTravail>> => {
      // const { data } = await api.get(BASE_PATH, { params: _filters });
      // return data;
    ...
    create: async (
      moyenDeTravail: MoyenDeTravailFormData
    ): Promise<MoyenDeTravail> => {
...
// Export de l'ancien service pour compatibilité (si nécessaire)
export const MoyensDesTravailService = {
  getAll: async (
    _filters: MoyensFilters // eslint-disable-line @typescript-eslint/no-unused-vars
  ): Promise<PaginatedResponse<MoyenDeTravail>> => {
    // Données mockées pour compatibilité
    const data: MoyenDeTravail[] = [
      {
        id_moyens_de_travail: 1,
        denomination: "Ordinateur portable",
  ...
  getById: async (
    ...
    throw new Error(
      "Utilisez useMoyensDeTravailService() dans un composant React"
    );
  },

  create: async (
    _moyenDeTravail: MoyenDeTravailFormData // eslint-disable-line @typescript-eslint/no-unused-vars
  ): Promise<MoyenDeTravail> => {
    throw new Error(
      "Utilisez useMoyensDeTravailService() dans un composant React"
    );
