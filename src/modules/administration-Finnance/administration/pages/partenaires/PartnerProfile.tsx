...
const ModernPartnerProfile: React.FC = () => {
  ...
  const { fetchContratsByPartenaire } = useContratsApi();
  ...
  const { mutate: addInterlocuteurMutation } = useMutation({
    mutationFn: (data: Omit<Interlocuteur, "id_interlocuteur">) =>
      addInterlocuteur(data),
    onSuccess: () => {
  ...
  const { mutate: deleteInterlocuteurMutation } = useMutation({
    mutationFn: (id_interlocuteur: number) =>
      deleteInterlocuteur(id_interlocuteur),
    onSuccess: () => {
  ...
  const removeInterlocuteur = async (id_interlocuteur: number) => {
    toast.custom(
      (t) => (
        <div>
          <div className="mb-2">
            Voulez-vous vraiment supprimer cet interlocuteur ?
          </div>
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <Button
            onClick={() => queryClient.invalidateQueries(["partenaire", id])}
  ...
  const handleEditClick = () => {
    ...
    navigate(`/gestion-administrative/partenaires/${id}/editer`);
  };

  const getInitials = (name: string) => {
    if (!name) return "erreur de nom";
    return name
      .split(" ")
      .map((word) => word[0])
      .join("")
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 -mt-4 rounded-lg shadow-md">
        <Tabs defaultValue="profil" className="mb-6">
          <TabsList
            className="w-full flex-nowrap overflow-x-auto whitespace-nowrap gap-1 px-1"
            style={{ scrollbarWidth: "thin" }}
          >
            <TabsTrigger
              value="profil"
              className="min-w-[120px] px-2 py-1 text-sm"
            >
              Profil
            </TabsTrigger>
            <TabsTrigger
                  ...
                  <CardContent className="p-6">
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          removeInterlocuteur(interlocuteur.id_interlocuteur)
                        }
                      >
                        <X size={16} />
                      </Button>
              ...
              <Button
                onClick={() =>
                  navigate("/gestion-administrative/contrats/nouveau", {
                    state: { partenaireId: partnerInfo.id_partenaire },
                  })
                }
                  ...
                  >
                    <CardContent className="p-4">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          navigate(
                            `/gestion-administrative/contrats/${contrat.id_contrat}`
                          )
                        }
                      >
                        Voir
                ...
                {interventions && interventions.length > 0 ? (
                  interventions.map((item: TypeInterventionPartenaire) => (
                    <Card
                      ...
                      onClick={() =>
                        navigate(
                          `/technique/interventions/${item.intervention.id_intervention}`
                        )
                      }
                    >
                      <CardContent className="p-6">
                          ...
                          {item.contrat && item.contrat.nom_contrat ? (
                            <span
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                navigate(
                                  `/administration/contrats/${item.contrat.id_contrat}`
                                );
                              }}
                            >
                        ...
                        <div className="mb-2">
                          ...
                          {item.employes && item.employes.length > 0 ? (
                            item.employes.map((emp) => (
                              <span
                                key={emp.id_employes}
                                className="inline-block mr-2 text-xs text-gray-700 bg-gray-100 rounded px-2 py-0.5"
                              >
