...
const formSchema = z.object({
  id_produit: z.string(),
  Code_produit: z.string()
    .regex(/^[A-Z]{3}-[A-Z]{3}-[A-Z]{3}$/, { message: "Le code produit doit être au format XXX-XXX-XXX" }),
  ...
  Qte_produit: z.coerce.number().min(0, { message: "La quantité doit être positive" }),
  emplacement: z.string().min(1, { message: "L'emplacement est requis" }),
  Id_type_produit: z.string().min(1, { message: "Le type de produit est requis" }),
...
function generateProductCode(modele: string, famille: string, marque: string): string {
...
export default function ProduitEdit() {
  ...
  React.useEffect(() => {
    if (selectedModeleId && selectedFamilleId && selectedMarqueId) {
      const selectedModele = modeleOptions.find(m => m.id === selectedModeleId)?.name || '';
      const selectedFamille = familleOptions.find(f => f.id === selectedFamilleId)?.name || '';
      const selectedMarque = marqueOptions.find(m => m.id === selectedMarqueId)?.name || '';
      
      const generatedCode = generateProductCode(selectedModele, selectedFamille, selectedMarque);
  ...
  return (
    <div className="container mx-auto py-4">
      <div className="flex items-center mb-6">
        <Button onClick={()=>navigate(-1)} variant="ghost" size="sm" className="mr-2">
          <ArrowLeft className="h-4 w-4 mr-1" />
          Retour
        </Button>
        <h1 className="text-2xl font-bold">Modifer un produit</h1>
      </div>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="space-y-2">
                          <FormLabel>Code Equipements *</FormLabel>
                          <FormControl>
                            <Input 
                              readOnly
